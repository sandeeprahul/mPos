...
import org.innoviti.innoviti.Wrapper;
...
public class PaymentActivity extends AppCompatActivity implements Inno_SerialComm_Process_EDC.QRCodePaymentInterface, View.OnClickListener {
    ...
    private EditText paymnt_cash, payment_card, coupon_code, coupon_amt, loyalty_amt, wallet_amt, redeem_code, loyalty_point;
    private TextView payable, Bal, BalPoint;
    private Button pay, apply, otp, initiate, cancel_bill, new_bill, manual_pay;
    ...
    private String fromActivity = Constants.PAYMENT;
            ...
            switch (msg.what) {
                case DISMISS_WINDOW:
                    ...
                    if (!responseResult.equals("")) {
                        ...
                        Toast.makeText(PaymentActivity.this, resCode + " : " + resMsg, Toast.LENGTH_LONG).show();
                        Log.i(TAG, resMsg);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editor = sp.edit();
        bal_amt = sp.getString("Total_Amt", "");
        ...
        Log.w(TAG, "Payment Bill Number " + bill_no);
        Log.w(TAG, "Payment Bill Amount " + billamt);
        ...
        initiate = findViewById(R.id.btnInitiate);
        loyalty_amt = findViewById(R.id.txtlyty);
        paymnt_cash = findViewById(R.id.txtcash);
        ...
        manual_pay = findViewById(R.id.btncardManual);
        ...
        if (isWallet.equalsIgnoreCase("Y")) {
            ...
            paymnt_cash.setText(sp.getString("Cashamt", ""));
        ...
        */
        ...
        apply.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                View v = PaymentActivity.this.getCurrentFocus();
                if (v != null) {
                ...
                if (net.isConnectingToInternet()) {
                    if (!coupon_code.getText().toString().isEmpty()) {
                        idialog = ProgressDialog.show(PaymentActivity.this, "", "Applying Coupon...", true);
                        handler = new Handler();
                    ...
                    alert.alert(PaymentActivity.this, "No Internet Connection",
                            "Please check " +
                    ...
                    new AlertDialog.Builder(PaymentActivity.this)
                            ...
                            .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int id) {
                                    ...
                                    editor.clear();
                                    editor.apply();
                                    Intent intent = new Intent(PaymentActivity.this, LoyaltyActivity.class);
        ...
        manual_pay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                SpnrCard.setOnItemSelectedListener(new CustomOnItemSelectedListener());
                    ...
                    if (Balance > 0) {
                        ...
                        if (edcDetails.size() > 0) {
                            if (edcDetails.get(0).getEdcID().toString().equalsIgnoreCase("2")) {
                                ...
                                idialog = ProgressDialog.show(PaymentActivity.this, "",
                        ...
                        Toast toast = Toast.makeText(PaymentActivity.this, "Balance Amount is zero.", Toast.LENGTH_SHORT);
                        toast.show();
        ...
        if (loyaltyCredentials!=null && loyaltyCredentials.size()==0){
                ...
                public void onClick(View view) {
                    ...
                    if (net.isConnectingToInternet()) {
                        ...
                        if (!loyalty_point.getText().toString().isEmpty()) {
                            max_amount = 0;
                            update_amount();
                            idialog = ProgressDialog.show(PaymentActivity.this, "", "Applying Loyalty...", true);
                            handler = new Handler();
                            handler.postDelayed(myRunnable = new Runnable() {
                                @Override
                                public void run() {
        ...
        pay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Totalamt = Couponamt + cashamt + Creditamt + Lyltyamt + Walletamt;
                Otheramt = Totalamt - cashamt;

                cashreturn = Totalamt - billamt;


                Totalamt = Totalamt - cashreturn;

                if (cashreturn < 0) {
                    idialog.dismiss();
                    handler.removeCallbacks(myRunnable);
                    showFailedAlert("Total amount received can't be less than bill value.");

                } else if (Math.round(cashreturn) > 0 && Math.round(cashamt) <= 0) {
                    idialog.dismiss();
                    handler.removeCallbacks(myRunnable);
                    showFailedAlert("Total amount received by credit card/coupon/Loyalty/Wallet can not be more than the total bill amount.");
                } else if (Lyltyamt > 0 && !redeem_code.getText().toString().equalsIgnoreCase(otp_code)) {

                    idialog.dismiss();
                    handler.removeCallbacks(myRunnable);
                    showFailedAlert("Entered OTP for loyalty redemption is not matching.");
                    ...
                    try {
                        ...
                        headerjsonobj.put("cash_amount_taken", cashamt);
                        headerjsonobj.put("cash_amount_returned", cashreturn);
                        headerjsonobj.put("other_amount", Otheramt);
                        headerjsonobj.put("total_payment_value", Totalamt);
                    ...
                    if (Math.round(cashamt) > 0) {

                        try {
                            cashamt = cashamt - cashreturn;
                            JSONObject detailjsonobj = new JSONObject();
                            detailjsonobj.put("payment_mode_id", "1");
                            detailjsonobj.put("cc_auth_code", "");
                            detailjsonobj.put("card_code", "");
                            detailjsonobj.put("cc_edc_no", "");
                            detailjsonobj.put("payment_value", cashamt);
                    ...
                    if (Math.round(Creditamt) > 0 & (Math.round(CardAmount) <= 0)) {

                        try {
                            JSONObject detailjsonobj = new JSONObject();
                            detailjsonobj.put("payment_mode_id", "2");
                            detailjsonobj.put("cc_auth_code", ApprovalCode);
                            detailjsonobj.put("card_code", "");
                            detailjsonobj.put("cc_edc_no", tID);
                            detailjsonobj.put("payment_value", Creditamt);
                            detailjsonobj.put("cust_card_code", cardNO);
                            detailjsonobj.put("customer_name", "");
                            detailjsonarray.put(count, detailjsonobj);
                            count = count + 1;
                    ...
                    if (Math.round(CardAmount) > 0) {

                        try {
                            JSONObject detailjsonobj = new JSONObject();
                            detailjsonobj.put("payment_mode_id", "2");
                            detailjsonobj.put("cc_auth_code", authID);
                            detailjsonobj.put("card_code", "");
                            detailjsonobj.put("cc_edc_no", EDCtype);
                            detailjsonobj.put("payment_value", CardAmount);
                            detailjsonobj.put("cust_card_code", CardType);
                            detailjsonobj.put("customer_name", "");
                            detailjsonarray.put(count, detailjsonobj);
                            count = count + 1;
                    ...
                    if (Math.round(Couponamt) > 0) {
                        ...
                        try {
                            ...
                            detailjsonobj.put("card_code", "");
                            detailjsonobj.put("cc_edc_no", "");
                            detailjsonobj.put("payment_value", Couponamt);
                            detailjsonobj.put("cust_card_code", "");
                            detailjsonobj.put("customer_name", "");
                            detailjsonarray.put(count, detailjsonobj);
                            count = count + 1;
                    ...
                    if (Math.round(Lyltyamt) > 0) {

                        try {
                            JSONObject detailjsonobj = new JSONObject();
                            detailjsonobj.put("payment_mode_id", "4");
                            detailjsonobj.put("cc_auth_code", "");
                            detailjsonobj.put("card_code", "");
                            detailjsonobj.put("cc_edc_no", "");
                            detailjsonobj.put("payment_value", Lyltyamt);
                            detailjsonobj.put("cust_card_code", "");
                            detailjsonobj.put("customer_name", "");
                            detailjsonarray.put(count, detailjsonobj);
                            count = count + 1;
                    ...
                    if (Math.round(Walletamt) > 0) {

                        try {
                            JSONObject detailjsonobj = new JSONObject();
                            detailjsonobj.put("payment_mode_id", "5");
                            detailjsonobj.put("cc_auth_code", "");
                            detailjsonobj.put("card_code", "");
                            detailjsonobj.put("cc_edc_no", "");
                            detailjsonobj.put("payment_value", Walletamt);
                            detailjsonobj.put("cust_card_code", "");
                            detailjsonobj.put("customer_name", "");
                            detailjsonarray.put(count, detailjsonobj);
                            count = count + 1;
                    ...
                    try {
                        ...
                        couponjsonobj.put("coupon_sl_no", couponSno.equalsIgnoreCase("") ? "" : couponSno);
                        if (bbEligibility.equalsIgnoreCase("Y")) {
                            ...
                            couponjsonobj.put("denom_value", Value > 0 ? Value / Count : "");

                        } else {
                            couponjsonobj.put("denom_count", Couponamt > 0 ? "1" : "");
                            couponjsonobj.put("denom_value", coupon_amt.getText().toString().equalsIgnoreCase("0.00") ? "" : coupon_amt.getText().toString());
                        }
                        couponjsonobj.put("lineNo", Couponamt > 0 ? "1" : "");
                    ...
                    if (wallets.size() > 0) {
                        for (int i = 0; i < wallets.size(); i++) {
                            try {
                                walletjsonobj.put("billno", bill_no);
                                walletjsonobj.put("wallet_id", wallets.get(i).getWalletID());
                                walletjsonobj.put("otp", wallets.get(i).getWalletOTP());
                                walletjsonobj.put("wallet_amount", wallets.get(i).getWalletAmount());
                                walletjsonobj.put("transId", wallets.get(i).getWalletTransID());
                                walletjsonarray.put(i, walletjsonobj);
                    ...
                    try {
                        ...
                        PBcouponjsonobj.put("coupon_sl_no", PBcode.equalsIgnoreCase("") ? "" : PBcode);
                        PBcouponjsonobj.put("coupon_code", "118");
                        PBcouponjsonobj.put("denom_count", "1");
                        PBcouponjsonobj.put("denom_value", PBamount.equalsIgnoreCase("0.00") ? "" : PBamount);
                        PBcouponjsonobj.put("lineNo", "1");
                        PBcouponjsonobj.put("location_code", Location);
                        PBcouponjsonobj.put("customer_phone", cust_mobile);
                        PBcouponjsonarray.put(0, PBcouponjsonobj);
                    ...
                    try {
                        ...
                        finaljsonobj.put("Wallet", walletjsonarray);
                        //if (fromActivity.equalsIgnoreCase(Constants.PROCESSING_BILL))
                        if (fromActivity.isEmpty())
                            finaljsonobj.put("couponpb", PBcouponjsonarray);

                    } catch (JSONException e) {
                        e.printStackTrace();
                    }

                    Log.w(TAG, "Payment Final JSON OBJ " + finaljsonobj);

                    RequestQueue requestQueue = AppController.getInstance().getRequestQueue();

                    String URL = "";
                    Log.w(TAG, "Bill Payment From Activity " + fromActivity);
                    //if (fromActivity.equalsIgnoreCase(Constants.PROCESSING_BILL))
                    if (fromActivity.isEmpty())
                        URL = PATH + "billpayment/";
                    else
                        URL = PATH + "manualbillpayment/";

                    Log.i(TAG, "Api Call: " + URL);

                    final String mRequestBody = finaljsonobj.toString();

                    Log.i(TAG, "final request body" + mRequestBody);
                    ConnectionDetector net = new ConnectionDetector(getApplicationContext());
                    if (net.isConnectingToInternet()) {
                        DisableButton();
                        StringRequest stringRequest = new StringRequest(Request.Method.POST, URL, new Response.Listener<String>() {
                            ...
                            public void onResponse(String response) {
                                ...
                                try {
                                    ...
                                    if (Status.equalsIgnoreCase("Success")) {
                                        ...
                                        bill_staus = true;
                                        if (Math.round(Creditamt) > 0) {
                                            Log.e(TAG, "Sending Transaction details..");
                                            idialog = ProgressDialog.show(PaymentActivity.this, "",
                                                    "Sending Transaction details..", true);
                                            SaveTransactionDetails(bill_no, resCode, resMsg, transDate, transTime, cardNO, tID, invNO, RRN, ApprovalCode);
                                        }
                            ...
                            .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int id) {
                                    ...
                                    if (net.isConnectingToInternet()) {
                                        ...
                                        RequestQueue queue = AppController.getInstance().getRequestQueue();
                                        final String url = PATH + "cancelbill?billNo=" + bill_no + "&location=" + Location;
                                        Log.i(TAG, "Api Call: " + url);
                                        JsonObjectRequest getRequest = new JsonObjectRequest(Request.Method.GET, url, null,
                                                new Response.Listener<JSONObject>() {
                                                    @Override
    ...
    public void LoadWalletScreen(View v) {
        ...
        if (bal > 0) {
            ...
            editor.putString("Cashamt", paymnt_cash.getText().toString());
    ...
    public void SaveTransactionDetails(String billNo, final String resCode, String resMsg, String transDate, String transTime, String cardNO, String tID, String invNO, String RRN, String ApprovalCode) {
        ...
        //Toast toast = Toast.makeText(PaymentActivity.this, "Transaction details saved successfully", Toast.LENGTH_SHORT);
        // toast.show();
    }
    ...
    public void onBackPressed() {
        if (IsSubmited) {
            showFailedAlert("Payment is on Process. You cannot go Back");
        } else {
            new AlertDialog.Builder(PaymentActivity.this)
    ...
    private void goBack() {
        if (!bill_staus) {
            if (!iswalletApplied) {
                doCloseEzetap();
                Intent i = new Intent(PaymentActivity.this, ProcessActivity.class);
    ...
    public void Send_ebill() {
        ...
        if (fromActivity != null && fromActivity.equalsIgnoreCase(Constants.MANUAL_BILL))
            url = PATH + "sentmbbebill?billNo=" + bill_no;
        else
            url = PATH + "sentebill?billno=" + bill_no + "&location=" + Location;

        Log.i(TAG, "Api Call SendEBill <<<:::>>>> " + url);
        JsonObjectRequest getRequest = new JsonObjectRequest(Request.Method.GET, url, null,
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
